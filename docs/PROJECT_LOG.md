...
|---|--------|------|-----------|
| 4 | feature/company-salon-onboarding | 2026-07-15 | B2B onboarding slice 1: POST `/api/business/companies` (ЕИК + checksum, PENDING_APPROVAL), POST `.../companies/{id}/salons`, POST `.../salons/{id}/services`, POST `.../salons/{id}/photos`. Auth от gateway headers + owner check. Flyway V3 fix demo EIK. Unit + WebMvc + integration тестове. CAS role/companyId update — отделен branch. |
